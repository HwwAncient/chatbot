...
class SequenceToSequence(object):
    ...
    def __init__(self,
        ...
        Args:
            ...
            time_major:
                ...
                注意，改变这个参数并不要求改变输入数据的格式, 但要调换输入数据的维度
                ...
                TensorFlow官方文档认为time_major=True会比较快, 具体见官方文档
            ...
            pretrained_embedding:
                True则代表有训练好的embedding，False代表需要自己从头开始训练embedding
        """
        ...
        # 生成随机分布的随机数，有四个参数：最小值、最大值、随机种子数（可以为空）、类型
        ...
        if share_embedding:
            assert input_vocab_size == target_vocab_size, '如果打开share_embedding，两个vocab_size必须一样'

        assert mode in ('train', 'decode'), 'mode必须是"train"或"decode"而不是"{}"'.format(mode)
    ...
    def build_model(self):
        ...
        3. 构建解码器（decoder） build_decoder -> decoder_cell -> build_single_cell
    ...
    def init_placeholders(self):
        ...
        # 有batch_size句话, 每句话是最大长度为time_step的index表示
        ...
        if self.mode == 'train':
            ...
            # 解码器的输入, shape=(batch_size, time_step)
            ...
            # 实际训练的解码器输入，实际上是start_token + decoder_inputs
            # 解码器的输入此处只加了<s>但是并没有去掉</s>
            # 但是并没有关系, 因为加上了<s>之后decoder_inputs_length并没有加1而是保持不变, 所以</s>将被mask掉
    ...
    def build_encoder(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            # embedded之后的输入shape=(batch_size, time_step, embedding_size)
    ...
    def build_decoder_cell(self, encoder_outputs, encoder_state):
        ...
        if self.use_beamsearch_decode:
            ...
            # 如果使用了beamsearch那么输入应该是beam_width倍于batch_size的
        ...
        # 在非训练（预测）模式，并且没开启beamsearch的时候，打开attention历史信息
        alignment_history = (
    ...
    def build_decoder(self, encoder_outputs, encoder_state):
        ...
        with tf.variable_scope('decoder') as decoder_scope:
            ...
            # Dense层没有设置激活函数就代表不进行激活函数非线性的操作
            ...
            if self.mode == 'train':
                ...
                # 训练的时候不在这里应用output_layer
                # 因为如果这里每个time_step都进行output_layer的投影计算会比较慢
                # 所以在结果出来后再统一进行该投影操作
                # 注意这个trick要成功必须设置dynamic_decoder的scope参数
                ...
                # 因为训练模式下知道decoder中每个输入的长度, 所以max_decoder_length
                # 和decoder_inputs_length中的最大长度设置成一样就好; 但是预测模式下
                # 不知道decoder_inputs_length, 所以需要人为设定max_decode_step
                ...
                # train_entropy不同于下面的loss是一个值, 这里没有进行loss的平均值计算, 是一个loss集合([batch_size, max_decoder_length]?)
                self.train_entropy = \
                ...
                start_tokens = tf.tile(
                    [WordSequence.START],
                    [self.batch_size]
                )  # 预测模式下decode的input不需要提前加开始标记, 本处指定start_tokens即可自动添上, (batch_size,)

                # end_token代表遇到该标记则句子的预测停止, (1,)
                ...
                if not self.use_beamsearch_decode:
                    ...
                    # 因为预测过程中下一个cell的输入是上一个cell的输出, 所以这个地方一定要有output_layer
                    # 而不能像训练模式下后面再统一进行output_layer投影操作
                ...
                (
                    ...
                    self.final_state,
                    _  # self.decoder_output_length_decode
                ) = seq2seq.dynamic_decode(
                ...
                if not self.use_beamsearch_decode:
                    ...
                    self.decoder_pred_decode = \
                        self.decoder_output_decode.predicted_ids
                    ...
                    # 现在再赋值给dod是之前操作可以改变self.decoder_output_decode的值
                    dod = self.decoder_output_decode
    ...
    def save(self, sess, save_path='model.ckpt'):
        """
        保存模型
        在Tensorflow中, 保存模型的格式有两种：
        ckpt: 训练后的模型保存, 这里面会保存所有的训练参数, 文件相对来讲比较大, 可以用来进行模型的恢复和加载
    ...
    def init_optimizer(self):
        """
        1. 初始化优化器
        支持的方法有sgd, adadelta, adam, rmsprop, momentum
        2. 梯度裁剪
        """
        ...
        # 梯度裁剪
        # 有三种更新方法, 对应于3种不同的loss
        # 为了节省空间, 三种方案内共用一些变量名, 只要self.update的名字不同就行, 因为最后只需要操纵它
    ...
    def check_feed(self, encoder_inputs, encoder_inputs_length,
        ...
        """
        给placeholder赋值时，可以对placeholder的变量名赋值也可以用Graph中的name赋值
        """
        ...
        if not decode:
            ...
            if target_batch_size != decoder_inputs_length.shape[0]:
                raise  ValueError(
                    "decoder_inputs和decoder_inputs_length的第一维度必须一致 "
                    "这一维度是batch_size, %d != %d" % (
    ...
    def train(self, sess, encoder_inputs, encoder_inputs_length,
        ...
        if add_loss is not None:
            ...
            # input_feed是一个字典用于填充占位符, output_feed是一个输出数组
        ...
        # 情况4(默认情况)
        # 默认对self.loss进行梯度下降, 跑self.updates更新self.loss
        output_feed = [
    ...
    def predict(self, sess,
        ...
        # 输入
        input_feed = self.check_feed(encoder_inputs,
                                     encoder_inputs_length, None, None, True)

        # 预测模式不需要dropout, 故设置keep_prob为1.0
