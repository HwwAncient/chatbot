...
def test(params):
    ...
    with tf.Graph.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            flow = ThreadedGenerator(
                # 训练时x_data作为encoder的输入不需加<end>, y_data作为decoder的输出标签需要加<end>
            ...
            # TODO [modify] 下次训练前先加载已训练的参数, 就可以接着上次的训练结果继续训练
    ...
    # 测试
    # SequenceToSequence()中不要parallel_iterations参数
    # 进行两次预测
    # 第二次的beam_width不用12用1就行了
    tf.reset_default_graph()
